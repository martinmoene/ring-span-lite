...
table = (
    ...
    ( 'CMakeLists.txt'
        , r'set\W+ring_span_lite_version\W+"([0-9]+\.[0-9]+\.[0-9]+)"\W+$'
        , 'set( ring_span_lite_version "{major}.{minor}.{patch}" )\n' )

#    , ( 'example/cmake-pkg/CMakeLists.txt'
#        , r'set\W+ring_span_lite_version\W+"([0-9]+\.[0-9]+(\.[0-9]+)?)"\W+$'
#        , 'set( ring_span_lite_version "{major}.{minor}" )\n' )
#
#    , ( 'script/install-xxx-pkg.py'
#        , r'\ring_span_lite_version\s+=\s+"([0-9]+\.[0-9]+\.[0-9]+)"\s*$'
#        , 'ring_span_lite_version = "{major}.{minor}.{patch}"\n' )
    ...
    , ( 'include/nonstd/ring_span.hpp'
        , r'\#define\s+ring_span_lite_MAJOR\s+[0-9]+\s*$'
        , '#define ring_span_lite_MAJOR  {major}' )

    , ( 'include/nonstd/ring_span.hpp'
        , r'\#define\s+ring_span_lite_MINOR\s+[0-9]+\s*$'
        , '#define ring_span_lite_MINOR  {minor}' )

    , ( 'include/nonstd/ring_span.hpp'
        , r'\#define\s+ring_span_lite_PATCH\s+[0-9]+\s*$'
        , '#define ring_span_lite_PATCH  {patch}\n' )
)
